...
mana_dev_configure(struct rte_eth_dev *dev)
{
	...
	for (unsigned int i = 0; i < (unsigned int)(2 * priv->num_queues); i++) {
		...
		DRV_LOG(DEBUG, "Register thread 0x%x for priv %p, port %u",
			i, priv, priv->port_id);
	}
...
mana_reset_exit(struct mana_priv *priv)
{
	...
	/*
	 * Uninstall the interrupt handler. Use the non-pending version
	 * because mana_reset_exit is called from the alarm timer callback
	 * where the MANA interrupt source is not active (only the alarm
	 * timerfd source is active). rte_intr_callback_unregister works
	 * when src->active == 0.
	 */
	...
	/*
	 * Spawn a thread for the heavy reset-exit work (IB close,
	 * re-probe, re-start) to avoid blocking the interrupt thread.
	 */
